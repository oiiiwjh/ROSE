...
请解析输入，识别：
1. **内容标识**：arxiv URL、arxiv ID（如 2401.12345）、alphaxiv URL（如 alphaxiv.org/overview/2401.12345）、本地 PDF 路径、通用 URL（非 arxiv/alphaxiv 的 http(s):// 链接，自动识别为 Blog 分支）、或已有内容的关键词
2. **可选参数**：
   ...
   - `--supplement <type:source>`: 补充信息源，可多次使用。类型包括：
     ...
     - `blog:https://...` — 博客文章
     - `video:https://...` — 视频讲解
   - `--type <content_type>`: 强制指定内容类型（blog / project-page / thread / newsletter），默认自动识别
...
1. **`library/papers/`**（已收藏论文）— 用 arxiv ID 前缀或标题关键词匹配目录名（如 `2401-12345*`）
2. **`library/blogs/`**（已收藏博客）— 用日期前缀或 slug 关键词匹配目录名（如 `20260404-llm-wiki*`），或用 URL/标题在 `meta.md` 中搜索
3. **`library/tmp/`**（临时内容）— 同上匹配规则（论文用 arxiv ID，博客用日期-slug）
4. **`library/daily/*.md`**（每日推荐记录）— 在所有日报中搜索该 arxiv ID 或论文标题关键词，提取已有的推荐理由、标签等信息
5. **`library/topics/*/paper_list.md`**（专题论文列表）— 搜索该论文是否已被某个研究专题收录

如果 arxiv ID 未命中，尝试用论文标题关键词在所有 `meta.md` 的 title 字段中模糊匹配。
如果输入为 URL，还需在 `library/blogs/` 和 `library/tmp/` 的 `meta.md` 中搜索 `url` 字段匹配。

**搜索方式**：使用 Glob + Grep 工具组合搜索，不要用 Bash 命令。例如：
- `Glob: library/papers/2401-12345*` 和 `Glob: library/tmp/2401-12345*` 匹配论文目录
- `Glob: library/blogs/*keyword*` 匹配博客目录
- `Grep: pattern="https://example.com/post" path="library/blogs/"` 按 URL 搜索博客
- `Grep: pattern="2401.12345" path="library/daily/"` 搜索日报提及
- `Grep: pattern="2401.12345" path="library/topics/"` 搜索专题提及

### Step 0b: 完整性检查

如果在 papers/、blogs/ 或 tmp/ 中找到了内容目录，检查以下文件的存在情况：

### Step 0b-1: Arxiv ID 有效性验证（论文类型，必须执行）

当本地找到论文的 `meta.md`（即 Step 0 命中了 papers/ 或 tmp/ 中的论文目录），且 `meta.md` 中有 `arxiv_id` 字段时，**必须**验证该 arxiv ID 是否仍然指向同一篇论文。

**验证方法**：
1. 运行 `python3 .claude/skills/read-paper/arxiv_fetch.py --id <arxiv_id>` 获取当前元信息
2. 将返回的 `title` 与本地 `meta.md` 中的 `title` 比较（忽略大小写和标点差异）
...
|---------|---------|
| 标题匹配 | 正常继续，使用本地数据 |
| 标题不匹配 | arxiv ID 已失效/重分配，需要修正 |
| 网络超时/无响应 | 跳过验证，继续使用本地数据（不阻断流程） |

**标题不匹配时的修正流程**：

1. 用论文标题关键词在 arxiv 搜索（`WebSearch: "论文标题关键词 arxiv"`），找到当前有效的 arxiv ID
2. 用新 ID 再次运行 `arxiv_fetch.py` 确认标题匹配
3. 通知用户 ID 已变更：
   ```
   ⚠️ Arxiv ID 变更检测：本地记录 2604.03741 → 当前有效 2604.03738
     本地标题: "Rethinking Position Embedding as a Context Controller..."
     2604.03741 当前指向: "Shower-Aware Dual-Stream Voxel Networks..."（不匹配）
     已确认 2604.03738 指向正确论文
   ```
4. 更新本地数据：
   - `meta.md` 中的 `arxiv_id` 和 `url` 字段
   - 目录名（从 `{old_id}-{slug}` 重命名为 `{new_id}-{slug}`）
   - `sources.md` 和 `analysis.md` 中引用旧 ID 的链接
   - `library/daily/*.md` 中引用旧 ID 的条目
5. 运行 `python3 .claude/skills/manage-library/generate_index.py` 刷新索引

**适用场景**：此验证在以下模式切换前执行：
- 分析模式（Step 0 → Step 0c 之间）
- Q&A 模式（Step 0 → Q&A Step 1 之间）

**注意**：此验证仅在本地命中论文目录时执行。如果用户直接输入 arxiv ID 且本地无记录，则无需验证（Step 1 获取的信息即为权威来源）。
...
根据输入和 `--type` 参数判断内容类型，决定后续处理分支：

| 输入 | --type 参数 | content_type | 分支 |
|------|------------|--------------|------|
| arxiv ID/URL/alphaxiv URL | 无 / paper | paper | Paper 分支（不变） |
| 本地 PDF | 无 / paper | paper | Paper 分支（不变） |
| 通用 URL（非 arxiv/alphaxiv） | 无 | 自动判断（blog / project-page / thread） | Blog 分支 |
| 任意 | blog / project-page / thread / newsletter | 指定类型 | Blog 分支 |

**自动判断规则**（按域名/路径模式匹配，非子串匹配）：
- 域名为 `x.com` 或 `twitter.com` → `thread`
- 域名为 `xiaohongshu.com` → `thread`
- 域名匹配 `*.github.io` 且路径不含 `/blog` → `project-page`
- 其他 URL → `blog`

进入 Blog 分支时，跳过 Step 1 的 AlphaXiv/arxiv_fetch 流程，转而执行 Step 1-blog。
...
2. **arxiv_fetch.py（AlphaXiv 返回 404 或超时时）**：
   ...
   - 此步仅获取元信息，不含全文分析
   - **ID 交叉验证**：如果本地已有 meta.md，将 `arxiv_fetch.py` 返回的 `title` 与本地 `title` 比较。如果不匹配，说明 arxiv ID 已变更，按 Step 0b-1 的修正流程处理（搜索正确 ID → 确认 → 更新本地数据）
...
### Step 1-blog: Blog 内容获取（仅 Blog 分支）

> **注意**：仅当 Step 0d 判定为 Blog 分支时执行此步骤，替代 Step 1 的 AlphaXiv/arxiv_fetch 流程。

**信息获取流程**：

1. **WebFetch 抓取页面内容**：
   - 使用 WebFetch 获取 URL 页面的完整 markdown 内容
   - 提取页面正文，忽略导航栏、页脚等非内容区域

2. **元信息提取**：
   - 从页面内容中提取：标题（title/h1）、作者（byline/author meta）、发布日期（date/pubdate meta）、所属站点
   - 如果页面元信息不完整，用 WebSearch 搜索该 URL 补充
   - 日期格式统一为 ISO `YYYY-MM-DD`，无法确定时使用抓取日期

3. **关联论文检测**：
   - 扫描页面中所有 arxiv 链接（匹配 `arxiv.org/abs/` 或 `arxiv.org/pdf/` 模式）
   - 提取 arxiv ID 列表，记录到 meta.md 的 `related_papers` 字段
   - 如果本地库已有相关论文（在 `library/papers/` 或 `library/tmp/` 中匹配），在分析时作为关联上下文

4. **WebSearch 补充**（可选，根据内容丰富度判断）：
   - 如果页面内容较短或信息不足，搜索 blog 标题/关键词获取补充讨论
   - 结果记录到 sources.md（复用 Step 1b 的格式）

**社交媒体特殊处理**：
- **X/Twitter thread**：识别 `x.com` 或 `twitter.com` URL，WebFetch 抓取完整 thread 内容
- **小红书**：识别 `xiaohongshu.com` URL，WebFetch 抓取

**Blog 分支的 meta.md 格式**（Step 3 中执行，此处定义差异）：

Blog 的 meta.md 无 `arxiv_id` 字段，用 `url` 替代；新增 `content_type`、`source_site`、`related_papers` 字段：
...
status: analyzed
source_site: "World Labs Blog"
related_papers: ["2401.12345"]
source: read-paper
collected_date: "2026-04-20"
---

## 概要总结

（中文 2-3 句话概括核心内容、方法和观点）

## 原文摘要

（从页面内容中提取的核心段落，保留原文语言）
...
- **Blog 分支目录命名**：无 arxiv ID 时使用 `{YYYYMMDD}-{slug}`，其中日期为发布日期（无法获取时用抓取日期），slug 从标题中提炼核心方法/主题的简短英文标识（2-5 词）
  - 示例：`20260415-claude-code-best-practices`、`20260410-worldlabs-scaling-3d`
- 如果 Blog 关联了已有论文（related_papers 中有论文且该论文已在本地库中），用户可选择将 blog 内容作为该论文的 supplement 存储（复用 `--supplement blog:` 机制），而非创建独立目录
...
**Blog 分支的模板适配**（当 `content_type != paper` 时）：

深度分析模板调整：
1. 「实验设计与结果分析」（四、实验结果与分析）→ 改为「实证与演示」：
   - 如果 blog 包含 benchmark/demo/定量结果：正常分析
   - 如果 blog 无实验内容：该段标注"本文未包含实验部分"，简短说明 blog 的验证方式（如代码示例、定性演示、案例分析）
2. 新增「关键 Insight」段（插入在「方法优势与深层分析」之后、「代码实现分析」之前）：
   - 提取 2-3 个作者的独特洞察、经验分享、非正式但有价值的观点
   - 这是 blog 相比论文的独特价值——非正式但关键的实践洞察
   - 格式：
     ```markdown
     ## X、关键 Insight

     - **Insight 1**：{洞察描述} — {为什么重要/非显而易见}
     - **Insight 2**：{洞察描述} — {为什么重要/非显而易见}
     ```

简略分析模板调整：
- 「主要贡献」→「核心内容」（措辞调整，blog 不一定有"贡献"）
- 「实验结论」→「实证与演示」（有则概括，无则标注"本文未包含实验"）
- 新增 1-2 句关键 Insight 概括
...
选项：
- **收藏**：根据 `content_type` 将目录移动到对应位置
  - `content_type: paper` → `library/papers/`
  - `content_type` 为 blog/project-page/thread/newsletter → `library/blogs/`
...
如果当前内容目录下**没有 `qa.md`**（即用户尚未精读），使用 AskUserQuestion 询问：

> 是否将这篇内容加入阅读列表，安排后续精读？

选项：
- **🔴 高优先级加入**：加入 `library/reading_list.md`，优先级为高
- **🟡 中优先级加入**：加入 `library/reading_list.md`，优先级为中
- **⚪ 低优先级加入**：加入 `library/reading_list.md`，优先级为低
- **跳过**：不加入阅读列表

如果用户选择加入，将内容按优先级追加到 `library/reading_list.md` 表格中（同一优先级内按收集日期升序排列）。备注栏填写一句话概括核心内容。链接格式使用相对路径（如 `[Title](papers/2401-12345-nerf/)`）。
...
定位内容目录（可能在 `library/papers/`、`library/blogs/` 或 `library/tmp/` 中），读取以下文件作为回答依据：
- `meta.md` — 元信息和摘要
- `analysis.md` — 详细分析
...
qa.md 写入完成后，检查 `library/reading_list.md` 中是否包含当前论文（按 arxiv ID 或目录名匹配）：
- 如果匹配到，从表格中移除该行，输出："✓ 已从阅读列表移除"
- 如果未匹配到，不输出任何内容
...
如果内容已在 `library/papers/` 或 `library/blogs/` 中，跳过此步。
